...
export default function ShareButtons({ title, url, className = '' }) {
  ...
  const platforms = [
    {
      ...
      icon: (
        <svg viewBox="0 0 24 24" fill="currentColor" width="14" height="14">
          <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.734l7.737-8.835L1.254 2.25H8.08l4.253 5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
        </svg>
      ),
      href: `https://twitter.com/intent/tweet?text=${shareTitle}&url=${encodedUrl}&via=MonocryNews`,
    },
    {
      key: 'telegram',
      label: 'Telegram',
    ...
    },
    {
      ...
      href: `mailto:?subject=${shareTitle}&body=Check this out: ${shareUrl}`,
    },
  ]
  ...
  const handleShare = (href, key) => {
    window.open(href, `share-${key}`, 'width=600,height=400,noopener,noreferrer')
  }
  ...
  return (
    <div className={`flex flex-wrap gap-2 ${className}`}>
      {platforms.map(p => (
        <button
          key={p.key}
          onClick={() => handleShare(p.href, p.key)}
          className={`share-btn ${p.key}`}
